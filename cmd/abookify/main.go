...
func main() {
	...
	// One-time metadata cleanup (idempotent): strip ASIN/codec junk from titles,
	// then fix authors that are chapter labels ("Chapter 7") or blank (backfill
	// from the EPUB). No-ops once the library is clean.
	if n, err := library.HealNoisyTitles(store); err != nil {
		log.Printf("warning: title-noise heal failed: %v", err)
	} else if n > 0 {
		log.Printf("healed %d noisy work title(s)", n)
	}
	if n, err := library.HealWorkAuthors(store); err != nil {
		log.Printf("warning: author heal failed: %v", err)
	} else if n > 0 {
		log.Printf("healed %d work author(s)", n)
	}
	...
	// Propagate series + genre metadata from EPUBs up to their parent works.
	// Runs once per boot — idempotent (skips works that already have the value).
	worksList, _ := store.ListWorks()
	for _, w := range worksList {
		if w.Series != "" && w.Genre != "" {
			continue // both already set (manual edit or prior run)
		}
		for _, tf := range w.TextFiles {
			...
			if w.Genre == "" && meta.Genre != "" {
				if err := store.SetGenre(w.ID, meta.Genre); err != nil {
					log.Printf("set-genre for %q failed: %v", w.Title, err)
				} else {
					log.Printf("genre: %q → %q", w.Title, meta.Genre)
				}
			}
			break
		}
